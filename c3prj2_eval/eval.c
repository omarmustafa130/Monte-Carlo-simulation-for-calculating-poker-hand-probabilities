...
{
    unsigned int* match_counts_arr = malloc((hand->n_cards)*(sizeof(*match_counts_arr)));
    for(size_t i = 0; i<hand->n_cards; i++)
    {   
        match_counts_arr[i] = 1;
        for(size_t k = 0; k<hand->n_cards; k++)
        {
            if(i != k)
            {
                if(hand->cards[i]->value == hand->cards[k]->value)
                    match_counts_arr[i] += 1;
            }
            
        }
    }
    return match_counts_arr;
}
